...
def log_trade_open(
    ...
    *,
    ...
    if spread_at_entry is None and yes_bid is not None and yes_ask is not None:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (
                    ...
                    entry_price, avg_entry_price,
                    book_ok_at_entry, hit_volume_cap, spread_at_entry, mid_at_entry,
                    llm_score, llm_raw_json, strategy_id or os.environ.get("STRATEGY_ID", "LLM_CRYPTO_15M"),
                    os.environ.get("CONFIG_VERSION") or os.environ.get("CONFIG_HASH"),
                    sl_price, tp_price,
                ),
            )
            row = cur.fetchone()
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                WHERE trade_id = %s
                """,
                (exit_ts, exit_price, avg_exit_price, pnl_usd, pnl_pct, max_dd_pct, mae_pct, mfe_pct, exit_reason, trade_id),
            )
            conn.commit()
            if cur.rowcount:
