...
class PaperTrader:
    ...
    def execute_orders(
        ...
        for order in approved_orders:
            ...
            # Не сохранять SL/TP, похожие на clamp по умолчанию (0.01/0.99) — иначе при загрузке можем сработать ложный стоп
            if sl < 0.02 or sl >= limit_price - 1e-6:
                sl = None
            else:
                sl = round(sl, 4)
            if tp > 0.98 or tp <= limit_price + 1e-6:
                tp = None
            else:
                tp = round(tp, 4)
            ...
            if market_id not in self.positions:
                ...
                # opened_ts: нужен для тайм-стопа и диагностики времени удержания (даже в обычных сделках).
                self.positions[market_id]["opened_ts"] = time.time()
                if is_mm:
                    self.positions[market_id]["mm"] = True
            ...
            if pos.get("mm"):
                out["mm"] = True
                out["opened_ts"] = pos.get("opened_ts") or time.time()
            elif pos.get("opened_ts") is not None:
                out["opened_ts"] = pos.get("opened_ts")
    ...
    def close_positions(
        ...
        for item in to_close:
            ...
            closed.append({
                ...
                "sl_at_exit": pos.get("stop_loss_price"), "tp_at_exit": pos.get("take_profit_price"),
                "market_title": mt if isinstance(mt, str) else None,
            })
