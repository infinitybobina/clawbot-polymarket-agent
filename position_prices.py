...
def _best_bid_from_levels(levels) -> Optional[float]:
    """Return max bid price across levels (books may be unsorted)."""
...
async def get_position_prices(
    ...
    try:
        ...
        for i, result in enumerate(results):
            tid = token_ids[i]
            # gather(return_exceptions=True): при отмене корутины сюда попадает CancelledError.
            # - Если отменяют всю main_loop (Ctrl+C / shutdown), пробрасываем, чтобы выйти быстро и без «залипания».
            # - Если CancelledError прилетел как транзиент (Windows/aiohttp), считаем как no_book для этого token.
            if isinstance(result, asyncio.CancelledError):
                t = asyncio.current_task()
                if t is not None and getattr(t, "cancelling", lambda: 0)() > 0:
                    raise result
                no_book += 1
                prices[tid] = None
                continue
...
def _best_bid_from_book(book: dict) -> Optional[float]:
    ...
    if not bids:
        return None
    return _best_bid_from_levels(bids)
